...
3. Resolve every conflicted file **semantically** — read both sides, keep both intents. Never blanket `--ours`/`--theirs` a source file. Repo-specific rules:
   ...
   - **`lib/importmap/version.rb`**: take the HIGHER `VERSION` — a feature PR that opens a new minor bumps it, and a release tags it, so two branches can legitimately carry different numbers. Equal numbers with different text, or any `UPSTREAM_VERSION` difference outside a `sync:` PR, means something is off → stop and ask.
