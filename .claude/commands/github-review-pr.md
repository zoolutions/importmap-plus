...
3. Resolve every conflicted file **semantically** — read both sides, keep both intents. Never blanket `--ours`/`--theirs` a source file. Repo-specific rules:
   ...
   - **`lib/importmap/version.rb`**: two branches can legitimately carry different numbers — a feature PR that opens a new minor bumps `VERSION`, and a release tags it. So a higher number on the branch is only correct when the branch's own commits explain it (`git log origin/<base>..HEAD -- lib/importmap/version.rb` — `origin/<base>`, the ref step 2 fetched, because a stacked PR's base has no local branch and a local `main` may be stale): a feature that opens that minor, or `bin/release`. Explained → take the higher. Unexplained (a bad merge, a stray edit), equal numbers with different text, or any `UPSTREAM_VERSION` difference outside a `sync:` PR → take the base's and stop to ask. Never propagate a bump nothing accounts for; `bin/release` would tag it.
