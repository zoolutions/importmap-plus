...
# What pin does about a package that can't be served as the single file an
# import map entry points at: the check, the file graph it downloads instead,
# the reasons that keep a package remote, and how doctor proves the result.
class Views::Docs::Pages::MultiFilePackages < DocsUI::Page
  title "Multi-file packages"
  eyebrow "Vendoring"

  def lead = "A vendored package is one file under a digested asset path. Plenty of packages need more than that, and pin reads every download before it writes one."
  ...
  def the_problem
    DocsUI::Section("Why one file isn't always enough",
                    description: "A digested asset path is not the directory the package was published to.") do
      md <<~'MD'
        [`pin`](/docs/pinning) downloads a package to `vendor/javascript/<package>.js`
        and the asset pipeline serves it under a digested path. That works as long as
        the file is self-contained. Plenty of published packages aren't: the entry
        point imports a sibling by relative path, spawns a worker, reads
        `import.meta.url` to find its own directory, computes an `import()`, or
        fetches a `.wasm` binary beside itself.

        Every one of those references is resolved by the browser against the file's
        own URL — which is the digested asset path, not the CDN directory the package
        was published to. Neither Propshaft nor Sprockets rewrites `import`
        statements, so the request goes out for a file that was never vendored and
        comes back 404. Nothing warns you before the page.

        So `pin` reads what it downloaded before it writes anything to
        `vendor/javascript`, and takes one of three routes: vendor the entry alone,
        vendor it together with [the file graph it needs](#vendoring-the-file-graph),
        or [leave the package on its CDN](#packages-kept-remote) and record why.
      MD
    end
  end

  def what_pin_looks_for
    DocsUI::Section("What the check looks for",
                    description: "Five patterns, all of them reported.") do
      md <<~'MD'
        `relative imports` is the one a download can answer for itself, by bringing
        the files along. The other four have no answer: they name a path nobody can
        rewrite, so the package stays on its CDN.
      MD
      DocsUI::Table(
      ...
      md <<~'MD'
        ...
        the browser. [`--vendor`](#overriding-the-check) is the way to say you know
        better.
      MD
    end
  end

  def vendoring_the_graph
    DocsUI::Section("Vendoring the file graph",
                    description: "The whole graph is downloaded, and one pin_all_from line maps it.") do
      md <<~'MD'
        Everything an entry reaches through relative imports lives under the
        package's own version directory on the CDN, which is a closed, finite set.
        `pin` crawls it, and rewrites every relative specifier to the bare key
        `<package>/<path without the extension>`:
      MD
      ...
      md <<~'MD'
        ...
        its own — and `date-fns` about 250. A directory also wins over a `pin` of the
        same key, so pinning a subpath of a package you have vendored this way has no
        effect — `pin` says so when it happens. `pin <package> --remote` is the way
        back to a single CDN URL.
        ...
        yours: `pin` skips the package, says so, writes nothing and exits non-zero
        rather than renaming it away — the same as when a CDN fails partway through a
        crawl, where the pin and whatever files it had are left exactly as they were.
        `unpin` takes the directory and its line along with the pin, and
        [`--vendor`](#overriding-the-check) drops both.
  ...
  def kept_remote
    DocsUI::Section("Packages kept remote",
                    description: "Pinned to the CDN instead of downloaded, with the reason on the pin.") do
      md <<~'MD'
        A download the file graph can't answer for — any reason but `relative
        imports`, or a graph that couldn't be taken over whole — is pinned to its CDN
        URL instead, and the pin comment records why:
      MD
      DocsUI::Code(<<~SHELL, lexer: :console)
      ...
      md <<~'MD'
        ...
        get. The `remote:` detail and the reason are part of the
        [provenance grammar](/docs/provenance), which means later commands can read
        them back.

        From then on the pin behaves like any other
        [remote pin](/docs/pinning#pinning-to-a-remote-url): `pin` and `update`
  ...
  def overriding_the_check
    DocsUI::Section("Overriding the check",
                    description: "--vendor downloads it anyway; --remote skips the download entirely.") do
      DocsUI::Code(<<~SHELL, lexer: :console)
  ...
  def proving_it
    DocsUI::Section("Proving it before the browser does",
                    description: "doctor reads the map and the files, offline.") do
      md <<~'MD'
        The check runs when a package is pinned. What it can't see is an app that was
        assembled over years — a package vendored before the check existed, a file
        hand-edited, a pin whose sibling directory somebody deleted.
        [`doctor`](/docs/updating#doctor) is that half: it boots the app and checks
        the import map against the files that are actually there, and a vendored file
        still importing a sibling by relative path is one of the things it reports —
      ...
      SHELL
      md <<~'MD'
        It exits non-zero on an error, so it belongs in
        [CI](/docs/updating#checking-the-app-in-ci) — which is where a 404 that only
        happens on one page is cheapest to find.
      MD
    end
  end
end
