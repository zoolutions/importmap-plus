...
| `pristine` | `--env/-e` ("production"), `--from/-f` (nil → each pin's own CDN), `--minify` (nil → each file's own state) | `Skipping "..." (pinned to remote URL)`; `Skipping "..." (locked at ..., CDN resolved ...)`; `Downloading "#{package}" to #{vendor_path}/#{package}.js from #{url}#{" (minified)" if minify}` | re-downloads vendored files; rewrites provenance comment only if provider/minified changed | none explicit |
