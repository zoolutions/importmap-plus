...
### A download that is skipped — the colon form `Skipping "<pkg>": …` — fails `pin`, `update` and `pristine`, even though the other packages were pinned
- **Holds because:** a package whose directory is in the way (`VendoredGraph::Occupied`) or whose CDN failed mid-crawl (`Packager::Error`) is deliberately left exactly as it was — but `return puts …` from that rescue made the command exit 0, so `bin/importmap pin x && git commit -am "pin x"` committed a tree with no pin in it. Every other command in the file that can't do all it was asked exits 1 (`lock`, `unlock`, `update`, `pristine`), and coding-style.md says a CLI command that cannot do what was asked says so *and* exits non-zero. The two rescues call `skip`, which prints `Skipping "<pkg>": …` and records the package on the command; `pin`, `update` and `pristine` end with `exit 1 if … skipped.any?`. The record lives on the command rather than in `pin_package`'s return value because an esm.run bundle's dependencies are pinned two calls down — from `pin`, `update` *and* `pristine` — and are skipped the same way. It is reconciled with a later success: two bundles can need one dependency, skipped on the first bundle's turn and pinned on the second's, and `update_importmap_with_pin` drops it from the record, so a run that ends with every pin in the tree exits 0.
- **Where:** `lib/importmap/commands.rb#skip`, `#skipped`, `#pin`, `#update`, `#pristine`, `#pin_vendored_package`, `#update_importmap_with_pin`
...
- **Proven by:** `test/commands_test.rb:"pin command skips a package whose directory the import map doesn't map"` (`run_importmap_command_expecting_failure`). The `pristine` path (an esm.run dependency whose CDN fails during a restore) has no live test: it needs a CDN to 503 on one URL and not another, which the command tests can't stage.
- **Origin:** gate rounds 5 and 6 (correctness), PR #30

### Not a bug: `Skipping "<pkg>" pinned to custom URL …` and `Keeping "<pkg>" pinned to … (couldn't resolve it from <provider>)` exit 0
- **Holds because:** both predate the file graph (`repin_remote_package`, upstream-adjacent) and describe a pin the command decided *not to touch* for a reason the pin itself carries — a URL this gem never resolves, or a provider the pin names that couldn't answer. Nothing the user asked for is missing from the tree afterwards; the pin is as it was and was never going to be vendored. The colon form above is different: the user asked for a download and didn't get one. Routing these two through `skip` would fail every `update` in an app with one custom-URL pin.
- **Where:** `lib/importmap/commands.rb#repin_remote_package`
- **Origin:** gate round 6 (correctness), PR #30
