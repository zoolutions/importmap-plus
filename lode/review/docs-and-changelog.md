...
- **Origin:** cubic on PR #26 (twelve findings, all this shape, after three clean gate rounds); gate on PR #30 (the integrity qualifiers)

### A hand-written list in `CLAUDE.md`, the rules or the profile is recomputed from the tree before it ships
- **Holds because:** these files are read as fact by every session. PR #30 shipped four stale lists in one pass: `CLAUDE.md`'s model-tier sentence named `fable`, a tier only the retired `/plan` pinned (the five surviving commands pin `opus` and `sonnet`); the profile's class-level accessor list named three of the five `singleton_class.attr_*` in `lib/` (`Packager.esm_run_resolver` and `Packager.minifier` missing, and a test sets the latter); `testing.md`'s fixture list named five of twelve files, and its rewrite described a 2×2 grid of outdated fixtures where only three exist; `lode-map.md`'s profile entry omitted the heading the same PR added. Each is one `grep model: .claude/commands/*.md`, `grep singleton_class lib/`, `ls test/fixtures/files/` or `grep '^## ' lode/workflow.md` away.
- **Where:** `CLAUDE.md`, `.claude/rules/*.md`, `lode/workflow.md`, `lode/lode-map.md`
- **Proven by:** no test — `/lode:gate`'s claims and correctness agents count each list against its source
- **Origin:** cubic on PR #30 (the `fable` clause); gate 2026-09-15 rounds 1–3 (the other three)
