...
| `package_graph_test.rb` | none — a hash of URLs stands in for the CDN | `Importmap::PackageGraph`: the crawl, the specifier rewrite, one row per refusal (escaping path, missing sibling, non-JavaScript sibling, colliding keys, case collision, a sibling needing more than the map can give, a rewrite that didn't take), and that the keys it writes are the keys a real `Importmap::Map#pin_all_from` gives the directory it writes |
| `vendored_graph_test.rb` | none | `Importmap::VendoredGraph`: the line it renders, that the line is found by its own directory and comment and by no pin regex, `mapping_for`/`conflict_in`/`entry_urls`, and that a failed swap puts the directory the app has back |
