...
`vendored_package_path(package)` (packager.rb:447-449) joins `vendor_path` with `package_filename(package)` (`package.gsub("/", "--") + ".js"`) — the only place that builds a vendored path from user input, per coding-style.md. `save_vendored_package` (packager.rb:684-702) treats the entry file and its graph directory as one write unit. `write_entry_partial` (packager.rb:705-720) writes the `// <package>@<version> downloaded from <url>[ (minified)]` header plus the source with any `//# sourceMappingURL=` comment stripped (`remove_sourcemap_comment_from`) into a pid-suffixed partial (`<target>.<pid>.download`) beside the target, cleaning that partial up in its own `ensure` if the write dies; then `Importmap::VendoredGraph#write` prepares the directory's partial the same way; only once both exist are they committed — `VendoredGraph#commit` renames the old directory aside and the new one in (restoring the old one in an `ensure` if the second rename fails), and `commit_entry` (packager.rb:723-726) `File.rename`s the entry partial over the target, clearing a directory in the way first. The whole sequence sits in a `begin`/`ensure` keyed on a `committed` flag, and every cleanup on this path is an `ensure` rather than a `rescue` because Ctrl-C through a 250-file minify is an `Interrupt`, not a `StandardError`. A failure anywhere before the renames leaves the files an app already had in place and no partial behind (test/packager_test.rb:"download leaves the file an app has when the replacement can't be written", :"download leaves no entry partial behind when the write is interrupted"; test/vendored_graph_test.rb:"commit puts the directory the app has back when the swap is interrupted").
